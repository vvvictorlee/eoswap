		...
		eos.contract(oracleContract)
			.then((contract) => {
				...
				for (let i = begin; i <= end; i++) {
					...
					console.log("$$$$push===",i,"Date.parse(new Date()) =", Date.parse(new Date()) );
					contract.pushdata({
						...
						.then(results => {
							...
							console.log("$$$$push result===",i,"Date.parse(new Date()) =", Date.parse(new Date()) );
						})
						.catch(error => {
...
function start_coin_timer() {
	...
	const update_cycle = 120;
	const duration = 30;
	const update_start_time = 1575535355;
...
function start_usd_timer() {
	...
	const update_cycle = 120;
	const duration = 30;
	const update_start_time = 1575535368;
