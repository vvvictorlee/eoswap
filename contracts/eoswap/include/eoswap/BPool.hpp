...
public:
  ...
  void initBPool(name _msg_sender, name factory,name pool_name) {
    _pool_storage.pools.insert(std::map<name,BPoolStore>::value_type(pool_name,BPoolStore())); 
    _pool_storage.pools[pool_name].controller = _msg_sender;
    _pool_storage.pools[pool_name].factory = factory;
  ...
  bool isBound(name t) { return _pool_storage.pools[pool_name].records[t].bound; }

  uint getNumTokens() { return _pool_storage.pools[pool_name].tokens.size(); }

  std::vector<name> getCurrentTokens() { return _pool_storage.pools[pool_name].tokens; }
  ...
  uint getDenormalizedWeight(name token) {
    require(_pool_storage.pools[pool_name].records[token].bound, "ERR_NOT_BOUND");
    return _pool_storage.pools[pool_name].records[token].denorm;
  }

  uint getTotalDenormalizedWeight() { return _pool_storage.pools[pool_name].totalWeight; }
  ...
  void setSwapFee(uint swapFee) {
    ...
    require(self == _pool_storage.pools[pool_name].controller, "ERR_NOT_CONTROLLER");
  ...
  {
    require(self == _pool_storage.pools[pool_name].controller, "ERR_NOT_CONTROLLER");
    require(!_pool_storage.pools[pool_name].records[token].bound, "ERR_IS_BOUND");
    require(!_pool_storage.pools[pool_name].finalized, "ERR_IS_FINALIZED");

    require(_pool_storage.pools[pool_name].tokens.size() < MAX_BOUND_TOKENS, "ERR_MAX_TOKENS");
  ...
  void rebind(name token, uint balance, uint denorm) {
    ...
    if (denorm > oldWeight) {
      _pool_storage.pools[pool_name].totalWeight = BMath::BMath::badd(
          _pool_storage.pools[pool_name].totalWeight, BMath::bsub(denorm, oldWeight));
      require(_pool_storage.pools[pool_name].totalWeight <= MAX_TOTAL_WEIGHT,
              "ERR_MAX_TOTAL_WEIGHT");
    } else if (denorm < oldWeight) {
      _pool_storage.pools[pool_name].totalWeight = BMath::bsub(_pool_storage.pools[pool_name].totalWeight,
    ...
    if (balance > oldBalance) {
      ...
      _pushUnderlying(token, _pool_storage.pools[pool_name].factory, tokenExitFee);
    }
  }
  ...
  void unbind(name token) {
    ...
    _pool_storage.pools[pool_name].totalWeight = BMath::bsub(
        _pool_storage.pools[pool_name].totalWeight, _pool_storage.pools[pool_name].records[token].denorm);
    ...
    _pool_storage.pools[pool_name].tokens[index] = _pool_storage.pools[pool_name].tokens[last];
    _pool_storage.pools[pool_name].records[_pool_storage.pools[pool_name].tokens[index]].index = index;
  ...
  void gulp(name token) {
    require(_pool_storage.pools[pool_name].records[token].bound, "ERR_NOT_BOUND");
    _pool_storage.pools[pool_name].records[token].balance = this->balanceOf(self);
  }

  uint getSpotPrice(name tokenIn, name tokenOut) {
    require(_pool_storage.pools[pool_name].records[tokenIn].bound, "ERR_NOT_BOUND");
    require(_pool_storage.pools[pool_name].records[tokenOut].bound, "ERR_NOT_BOUND");
  ...
  void joinPool(uint poolAmountOut, std::vector<uint> maxAmountsIn) {
    ...
    for (uint i = 0; i < _pool_storage.pools[pool_name].tokens.size(); i++) {
      ...
      _pool_storage.pools[pool_name].records[t].balance =
          BMath::badd(_pool_storage.pools[pool_name].records[t].balance, tokenAmountIn);
  ...
  void exitPool(uint poolAmountIn, std::vector<uint> minAmountsOut) {
    ...
    for (uint i = 0; i < _pool_storage.pools[pool_name].tokens.size(); i++) {
      ...
      _pool_storage.pools[pool_name].records[t].balance =
          BMath::bsub(_pool_storage.pools[pool_name].records[t].balance, tokenAmountOut);
  ...
  std::pair<uint, uint> swapExactAmountIn(name tokenIn, uint tokenAmountIn,
    ...
    uint tokenAmountOut =
        calcOutGivenIn(inRecord.balance, inRecord.denorm, outRecord.balance,
                       outRecord.denorm, tokenAmountIn, _pool_storage.pools[pool_name].swapFee);
  ...
  std::pair<uint, uint> swapExactAmountOut(name tokenIn, uint maxAmountIn,
    ...
    uint tokenAmountIn =
        calcInGivenOut(inRecord.balance, inRecord.denorm, outRecord.balance,
                       outRecord.denorm, tokenAmountOut, _pool_storage.pools[pool_name].swapFee);
  ...
  uint joinswapExternAmountIn(name tokenIn, uint tokenAmountIn,
    ...
    require(_pool_storage.pools[pool_name].finalized, "ERR_NOT_FINALIZED");
    require(_pool_storage.pools[pool_name].records[tokenIn].bound, "ERR_NOT_BOUND");
    require(tokenAmountIn <= BMath::bmul(_pool_storage.pools[pool_name].records[tokenIn].balance,
                                         MAX_IN_RATIO),
            "ERR_MAX_IN_RATIO");
    ...
    uint poolAmountOut = calcPoolOutGivenSingleIn(
        ...
        _pool_storage.pools[pool_name].totalWeight, tokenAmountIn, _pool_storage.pools[pool_name].swapFee);
  ...
  uint joinswapPoolAmountOut(name tokenIn, uint poolAmountOut,
    ...
    uint tokenAmountIn = calcSingleInGivenPoolOut(
        inRecord.balance, inRecord.denorm, _token_storage.totalSupply,
        _pool_storage.pools[pool_name].totalWeight, poolAmountOut, _pool_storage.pools[pool_name].swapFee);
  ...
  uint exitswapPoolAmountIn(name tokenOut, uint poolAmountIn,
    ...
    uint tokenAmountOut = calcSingleOutGivenPoolIn(
        ...
        _pool_storage.pools[pool_name].totalWeight, poolAmountIn, _pool_storage.pools[pool_name].swapFee);

    require(tokenAmountOut >= minAmountOut, "ERR_LIMIT_OUT");

    require(
        tokenAmountOut <=
            BMath::bmul(_pool_storage.pools[pool_name].records[tokenOut].balance, MAX_OUT_RATIO),
        "ERR_MAX_OUT_RATIO");
  ...
  uint exitswapExternAmountOut(name tokenOut, uint tokenAmountOut,
    ...
    uint poolAmountIn = calcPoolInGivenSingleOut(
        outRecord.balance, outRecord.denorm, _token_storage.totalSupply,
        _pool_storage.pools[pool_name].totalWeight, tokenAmountOut, _pool_storage.pools[pool_name].swapFee);
