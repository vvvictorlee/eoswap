...
class IUniswapV2Pair {
 public:
   ...
   virtual uint256 decimals() = 0;

   virtual uint256 balanceOf(address owner) = 0;
   ...
   virtual uint256 nonces(address owner) = 0;

   virtual void
   permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) = 0;

   virtual uint256 price0CumulativeLast() = 0;

   virtual uint256 liquidity kLast() = 0;

   virtual uint256 amount0, uint256 amount1 burn(address to) = 0;
...
class UniswapV2ERC20 {
 public:
   ...
   void _mint(address to, uint256 value) {
      totalSupply   = totalSupply.add(value);
      balanceOf[to] = balanceOf[to].add(value);
   }

   void _burn(address from, uint256 value) {
      balanceOf[from] = balanceOf[from].sub(value);
      totalSupply     = totalSupply.sub(value);
   }

   void _approve(address owner, address spender, uint256 value) { allowance[owner][spender] = value; }

   void _transfer(address from, address to, uint256 value) {
      balanceOf[from] = balanceOf[from].sub(value);
      balanceOf[to]   = balanceOf[to].add(value);
   }
...
class IUniswapV2Factory {
 public:
   ...
   void _safeTransfer(address token, address to, uint256 value) {
   ...
   void _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1) {
   ...
   uint256 liquidity mint(address to) {
      uint256 liquidity;
   ...
   // this low-level function should be called from a contract which performs important safety checks
   std::tuple<uint256, uint256> burn(address to) {
      uint256 amount0;
      uint256 amount1;
      (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
   ...
   // this low-level function should be called from a contract which performs important safety checks
   void swap(uint256 amount0Out, uint256 amount1Out, address to, bytes data) {
      lock();
