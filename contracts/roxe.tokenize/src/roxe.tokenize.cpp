...
void tokenize::create(const name& issuer, const asset& maximum_supply) {
...
void tokenize::transfer(const name& from, const name& to, const asset& quantity, const string& memo) {
...
void tokenize::setfee(const symbol& symbol, const int64_t fee) {
   ...
   const auto& st = *existing;
   statstable.modify(st, same_payer, [&](auto& s) { s.fee = fee; });
}
...
void tokenize::addauthor(const symbol& sym, const name& author) {
   ...
   statstable.modify(st, same_payer, [&](auto& s) {
      vector<name>::iterator iter = find(s.authors.begin(), s.authors.end(), author);
...
void tokenize::setfeeper(const symbol& symbol, const int64_t percent) {
   ...
   const auto& st = *existing;
   statstable.modify(st, same_payer, [&](auto& s) { s.percent = percent; });
}
...
void tokenize::setmaxfee(const symbol& symbol, const int64_t maxfee) {
   ...
   statstable.modify(st, same_payer, [&](auto& s) { s.maxfee = maxfee; });
}
...
void tokenize::setminfee(const symbol& symbol, const int64_t minfee) {
   ...
   statstable.modify(st, same_payer, [&](auto& s) { s.minfee = minfee; });
}
...
void tokenize::useroc(const symbol& symbol, const bool roc) {
   ...
   statstable.modify(st, same_payer, [&](auto& s) { s.useroc = roc; });
}
...
void tokenize::setfix(const symbol& symbol, const bool fix) {
   ...
   statstable.modify(st, same_payer, [&](auto& s) { s.fixed = fix; });
}
