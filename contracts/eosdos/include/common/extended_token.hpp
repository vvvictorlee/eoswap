...
struct [[eosio::table, eosio::contract("eosdos")]] account {
   uint64_t       sequence;
   extended_asset balance;

   uint64_t  primary_key() const { return sequence; }
   uint128_t by_extended_asset() const { return to_namesym(balance.get_extended_symbol()); }
};

struct [[eosio::table, eosio::contract("eosdos")]] currency_stats {
   asset supply;
   asset max_supply;
   name  issuer;
...
/**
 * extended_token contract defines the structures and actions that allow users to create, issue, and manage
 * tokens on EOSIO based blockchains.
 */
class extended_token {
 ...
 public:
    ...
    */
   void open(const name& owner, const extended_symbol& symbol, const name& ram_payer);
