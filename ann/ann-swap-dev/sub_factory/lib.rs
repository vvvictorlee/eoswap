...
pub use self::sub_factory::{SubFactory, SubFactoryRef};
...
pub mod sub_factory {
    ...
    use sub_pool::sub_pool::{FeeAmountEnabled, OwnerChanged, PoolCreated};
    #[derive(Default, scale::Encode, scale::Decode, SpreadAllocate, SpreadLayout, PackedLayout)]
    #[cfg_attr(
        feature = "std",
        derive(
            Debug,
    ...
    pub struct SubFactory {
        ...
        get_pool: Mapping<(AccountId, AccountId, u32), AccountId>,
        // parameters: Mapping<bool,Parameters>,
    ...
    // The SubFactory result types.
    pub type Result<T> = core::result::Result<T, Error>;

    impl SubFactory {
        /// Creates a new SubFactory contract.
        #[ink(constructor)]
        pub fn new() -> Self {
            ...
            ink_lang::utils::initialize_contract(|contract: &mut Self| {
                ...
                contract.original = Self::env().account_id();

                Self::env().emit_event(OwnerChanged {
                    old_owner: AccountId::from([0x0; 32]),
                    new_owner: Self::env().caller(),
                });
                for (fee, tick_spacing) in [(500, 10), (3000, 60), (1000, 200)] {
                    contract.fee_amount_tick_spacing.insert(&fee, &tick_spacing);
                    Self::env().emit_event(FeeAmountEnabled { fee, tick_spacing });
                }
            })
        }
        ...
        pub fn create_pool(
            ...
            ensure!(token_a != token_b, Error::TokenAEqualTokenB);
            let (token0, token1) = if token_a < token_b {
                ...
                token0 != AccountId::from([0x0; 32]),
                Error::Token0IsZeroAddress
            );
            let tick_spacing = self.fee_amount_tick_spacing.get(&fee).unwrap_or_default();
            ensure!(tick_spacing != 0, Error::TickSpacingIsZero);
            ensure!(
                self.get_pool
            ...
            let pool = self.deploy(self.env().account_id(), token0, token1, fee, tick_spacing)?;

            self.get_pool.insert(&(token0, token1, fee), &pool);
            // populate mapping in the reverse direction, deliberate choice to avoid the cost of comparing addresses
            self.get_pool.insert(&(token1, token0, fee), &pool);
        ...
        #[ink(message)]
        pub fn enable_fee_amount(&mut self, fee: u32, tick_spacing: i32) -> Result<()> {
            ensure!(self.env().caller() == self.owner, Error::OnlyOwner);

            ensure!(fee < 1000000, Error::FeeIsToLarge);
            ...
            // 16384 ticks represents a >5x price change with ticks of 1 bips
                ...
                Error::TickSpacingIsOutOfRange
            );
            ensure!(
                self.fee_amount_tick_spacing.get(fee).unwrap_or_default() == 0,
                Error::FeeAlreadyExist
            );

            self.fee_amount_tick_spacing.insert(&fee, &tick_spacing);
            self.env()
                .emit_event(FeeAmountEnabled { fee, tick_spacing });
            Ok(())
        }
        fn deploy(
            &mut self,
            ...
            ensure!(self.code_hash != Hash::default(), Error::CodeHashIsEmpty);
            // self.parameters.insert(&true,&Parameters {
            //     factory,
            //     token0,
            //     token1,
            //     fee,
            ...
            let instantiate_contract = || -> Result<AccountId> {
                ...
                #[cfg(not(test))]
                {
                    let mut version: Vec<u8> = factory.encode();
                    version.extend(token0.encode());
                    version.extend(token1.encode());
                    version.extend(fee.encode());
                    version.extend(tick_spacing.encode());
                    let salt = version;
                    let instance_params = sub_pool::SubPoolRef::new()
                        .endowment(0)
                    ...
                    let contract_addr =
                        init_result.expect("failed at instantiating the `pool` contract");
                    Ok(contract_addr)
                }
            };
            let pool = instantiate_contract()?;
            // self.parameters.remove(&true);
        ...
        ///  owner  owner
        #[ink(message)]
        pub fn parameters(&self) -> (AccountId, AccountId, AccountId, u32, i32) {
            let Parameters {
                factory,
                token0,
                token1,
        ...
        ///  code_hash  code_hash
        #[ink(message)]
        pub fn update_code_hash(&mut self, code_hash: Hash) {
    ...
    mod tests {
        ...
        fn init_contract() -> SubFactory {
            let erc = SubFactory::new(
                django(),
                eve(),
                frank(),
                1,
