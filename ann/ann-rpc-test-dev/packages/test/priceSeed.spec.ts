...
const priceSeed = require("../abi/sub_auction_v0.1.json");
...
beforeAll(async () => {
    ...
    // await api.isReady;
    // let abi = new Abi(priceSeed, api.registry.getChainProperties());
...
describe("priceSeed", () => {
    ...
    test("registerOracle", async () => {
        const oracleAddress = getContractAddress("oracle");
        await signedTxC("priceSeed", "registerOracle", [oracleAddress], null, null, null)
        await queryC("priceSeed", "oracles", [], null)
    ...
    test("updateOracle", async () => {
        setCaller(alicePair)
        const oracleAddress = getContractAddress("oracle");
        await signedTxC("priceSeed", "updateOracle", [oracleAddress], null, null, null)
        const a = await queryC("priceSeed", "oracles", [oracleAddress], null)
        console.warn("========priceSeed list item=======", a)
        expect(global.console.error).toHaveBeenCalledWith('log');

    });

    test("updateAddressRegistry", async () => {
        setCaller(alicePair)
        const addressRegistryAddress = getContractAddress("addressRegistry");
        await signedTxC("priceSeed", "updateAddressRegistry", [addressRegistryAddress], null, null, null)
        const a = await queryC("priceSeed", "addressRegistry", [], null)
        console.warn("========priceSeed list item=======", a)
        expect(global.console.error).toHaveBeenCalledWith('log');
