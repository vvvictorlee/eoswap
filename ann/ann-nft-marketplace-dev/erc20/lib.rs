...
mod erc20 {
    ...
    impl Erc20 {
        ...
        #[ink(constructor)]
        pub fn new(owner: AccountId, initial_supply: Balance) -> Self {
            // This call is required in order to correctly initialize the
            // `Mapping`s of our contract.
            ink_lang::utils::initialize_contract(|contract| {
                Self::new_init(contract, owner, initial_supply)
            })
        }

        /// Default initializes the ERC-20 contract with the specified initial supply.
        fn new_init(&mut self, owner: AccountId, initial_supply: Balance) {
            let caller = owner; //Self::env().caller();
